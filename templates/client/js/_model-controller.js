  ...
  .controller('<%= _.capitalize(name) %>Ctrl', ['$scope', '$modal', 'resolved<%= _.capitalize(name) %>', '<%= _.capitalize(name) %>',
    function($scope, $modal, resolved <%= _.capitalize(name) %> , <%= _.capitalize(name) %> ) {
      ...
      $scope.update = function(id) {
        $scope. <%= name %> = <%= _.capitalize(name) %> .get({
          id: id
        });
        $scope.open(id);
      };
      ...
      $scope["delete"] = function(id) {
        ...
        if (x) {
          <%= _.capitalize(name) %>["delete"](id).success(function(payload) {
            $rootScope.$broadcast("success", "<%= _.capitalize(name) %> has been deleted successfully!");
            return <%= _.capitalize(name) %>.search($location.search()).success(function(payload) {
      ...
      $scope.save = function(id) {
        if (id) { <%= _.capitalize(name) %> .update({
              id: id
            }, $scope. <%= name %> ,
            function() {
        ...
        } else { <%= _.capitalize(name) %> .save($scope. <%= name %> ,
            function() {
              $scope. <%= pluralize(name) %> = <%= _.capitalize(name) %> .query();
              $scope.clear();
            });
      ...
      $scope.clear = function() {
        $scope. <%= name %> = { <% _.each(attrs, function(attr) { %> "<%= attr.attrName.replace("
            ", "
            _ ").toLowerCase() %>": "",
            <%
          }); %>
